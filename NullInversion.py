    ...
    def null_optimization(self, latents, num_inner_steps, epsilon):
        ...
        for i in range(self.NUM_DDIM_STEPS):
            ...
            for j in range(num_inner_steps):
                ...
                #     "uncond_embeddings",
                #     "norm",
                #     uncond_embeddings.abs().sum().item(),
                #     "grad_norm",
                #     uncond_embeddings.grad.abs().sum().item(),
                # )
                # print(
                #     uncond_embeddings[0, :2, :2].detach().cpu().numpy(),
                # )
                # print(
                #     uncond_embeddings.grad[0, :2, :2].detach().cpu().numpy(),
                # )
                loss_item = loss.item()
                # print(f"inner_step_{j} loss:", loss_item)
                bar.set_postfix(dict(inner_step=j, loss=f"{loss_item:.8f}"))
                bar.update()
